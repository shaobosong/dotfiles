#!/bin/bash

# ci: Interactively change directory upwards
#

# Usage:
#   1. Add a alias (e.g. 'alias ci="source /path/cd-index.sh"') in ~/.bashrc
#   2. (Optional) Set keymap before sourcing: export CI_KEYMAP="emacs"
#   3. Run `source ~/.bashrc` or open a new terminal
#   4. Type `ci` in any nested directory
#
# Configuration:
#   - CI_KEYMAP: Set to "vim" (default) or "emacs" to change key bindings.
#

# Set default keymap if not configured by the user
: "${CI_KEYMAP:=vim}"

# ANSI escape codes for TUI rendering
HL_START="\e[7m"      # Highlight start (reverse video)
HL_END="\e[0m"        # Highlight end (reset)
CURSOR_HIDE="\e[?25l" # Hide cursor
CURSOR_SHOW="\e[?25h" # Show cursor
CLEAR_LINE="\e[K"     # Clear line from cursor to end

# Exit if in root directory
[[ "$PWD" == "/" ]] && return 0

# Split path into an array: /a/b/c -> (a b c)
path_parts=(${PWD//\// })
num_parts=${#path_parts[@]}
current_index=$((num_parts - 1))

# Restore terminal state
_cleanup() {
    printf "\r${CLEAR_LINE}"
}

# Key press down event handlers
_move_left()  { (( current_index > 0 )) && ((current_index--)); }
_move_right() { (( current_index < num_parts - 1 )) && ((current_index++)); }
_move_start() { current_index=0; }
_move_end()   { current_index=$((num_parts - 1)); }
_change_dir() {
    _cleanup
    # Build the target path from the selected index
    target_dir="/"
    for i in $(seq 0 $current_index); do
        target_dir+="${path_parts[i]}/"
    done
    cd "$target_dir"
}

# Renders the interactive path string
_render() {
    local display_path="/"
    for i in "${!path_parts[@]}"; do
        if [[ $i -eq $current_index ]]; then
            display_path+="${HL_START}${path_parts[i]}${HL_END}/"
        else
            display_path+="${path_parts[i]}/"
        fi
    done
    # \r moves cursor to line start, then we print and clear extra chars
    printf "\r${display_path}${CLEAR_LINE}"
}

_render

while true; do
    # Read a single character (-s: silent, -n 1: one char)
    # Using $'' syntax for control characters
    read -rsn1 key

    # Handle multi-byte sequences for Alt keys and arrows
    if [[ "$key" == $'\x1b' ]]; then # ESC character
        # Read next chars with a tiny timeout to see if it's a sequence
        read -rsn1 -t 0.01 next_key
        key+="$next_key"
        read -rsn1 -t 0.01 next_key
        key+="$next_key"
    fi

    # Key dispatcher based on the configured keymap
    if [[ "$CI_KEYMAP" == "emacs" ]]; then
        case "$key" in
            $'\x02' | $'\x1b[D' | $'\x1bb') _move_left ;;  # C-b | Left Arrow | Alt-b
            $'\x06' | $'\x1b[C' | $'\x1bf') _move_right ;; # C-f | Right Arrow | Alt-f
            $'\x01' | $'\x1b[H') _move_start ;; # C-a | Home
            $'\x05' | $'\x1b[F') _move_end ;;   # C-e | End
            '') _change_dir; return 0 ;; # Enter key
            'q' | $'\x1b') _cleanup; return 0 ;; # q, or ESC
        esac
    else # Default to "vim" keymap
        case "$key" in
            'h' | 'b' | $'\x1b[D') _move_left ;;  # h, b, Left Arrow
            'l' | 'w' | $'\x1b[C') _move_right ;; # l, w, Right Arrow
            '0' | '^' | $'\x1b[H') _move_start ;; # 0, ^, Home
            '$' | $'\x1b[F') _move_end ;;   # $, End
            '') _change_dir; return 0 ;; # Enter key
            'q' | $'\x1b') _cleanup; return 0 ;; # q, or ESC
        esac
    fi
    _render
done
